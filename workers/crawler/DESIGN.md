...
- **Domain Circuit Breaker (`domainCircuit`)** — Redis 記錄每個 domain 的連續失敗次數；3 次失敗開啟熔斷（30 分鐘冷卻）。`checkCircuit()` returns state (never throws); `evaluateCircuit()` (Bug #3 Sub C) is the gate helper used by `runPlaywright` lambda — when circuit is OPEN or another worker holds the probe, returns `{ kind: 'blocked', reason: 'CIRCUIT_OPEN' | 'CIRCUIT_HALF_OPEN' }` so orchestrator's `pickTrack` falls through to ScreenshotOne / cheerio rather than killing the whole job.
...
### 7. 在 worker handler 頂層用 throw 處理 circuit 狀態
若 worker handler 直接 `if (circuitState === 'open') throw new UnrecoverableError(...)`，BullMQ 會將整個 job 標為永久失敗 — orchestrator 的 fallback chain（ScreenshotOne / cheerio）**完全沒有機會執行**，等於把 fallback 設計徹底廢掉。Bug #3 Sub C 把 circuit 檢查搬進 `runPlaywright` lambda 並回傳 `{ kind: 'blocked' }`，pickTrack 自動往下試備用 track。Circuit breaker 仍然保護 IP（不會繼續打 playwright），但 user 仍可能透過 SaaS 拿到結果。
