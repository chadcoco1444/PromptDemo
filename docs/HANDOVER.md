...
測試數從 ~700 → 814（landing app 不含 unit tests，pure visual smoke）。Live: https://chadcoco1444.github.io/LumeSpec/ — Tally form `vG01xg` 已接、實際收 email 中。
...
| Landing v1.6 — Duolingo 第二排 | OPEN（候選）| 加 3 支 duolingo MP4 + 元件泛化收 (URL, intents)；驗證「不只 vercel 能 work」。~$0.30 + 1 hr |
