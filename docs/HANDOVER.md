...
| 2026-04-28 PM | Landing page → GitHub Pages（apps/landing/ Vite static + Hero + IntentMatrix + TallyEmbed + Footer）| 311c80b → 02f741f |
| 2026-04-28 PM | IntentVideoShowcase v1.5（vercel × 3 intents 並排 autoplay-on-scroll）| 2ada6af → 8b15caf |
| 2026-04-28 PM | normalizeText `& → and` symmetric fold（Burton tech extractive 撞牆 hot-fix）| c64324a |
| 2026-04-28 PM | IntentVideoShowcase v1.6 multi-URL 分組（vercel + burton 雙 row）| aeebc36 → 89d7488 |
| 2026-04-28 PM | IntentMatrix burton row 對齊 v1.6 + 測試數 stale 更新 + plan archive | eca7765 → 6664346 |
| 2026-04-28 PM late | **v1.7 Phase 1 視覺多樣性升級**（QuoteHero 新 scene + TextPunch photoBackdrop/slideBlock variants + SCENE_PACING_DISCIPLINE prompt + textPunchDiscipline soft validator + extractive joinedPool fast-path）| e635832 → 4f80667（10 commits, 2-stage 全 reviewed）|
| 2026-04-28 PM late | Re-render landing showcase 套 v1.7（6 MP4 + matrix sync）| 1445969 |
| 2026-04-28 PM late | **Phase 2 VersusSplit 新 scene type**（第 14 個 scene、左右對比 + 4 framing enum）| baf366f → 9684aa2（6 commits）|

測試數 ~700 → ~830+（landing app 不含 unit tests）。Live: https://chadcoco1444.github.io/LumeSpec/ — Tally form `vG01xg` 已接、實際收 email 中。Showcase 6 卡（2 brand × 3 intent）全部用 v1.7 variants 視覺多樣化。

### Backlog — 真實還開的項目

| 項目 | 狀態 | 卡關原因 |
|---|---|---|
| Bug #3 Sub A/B（anti-bot vendor）+ L4 geo proxy 合併採購 | **READY for $ decision** | 2026-04-28 vendor research done — 推 ScreenshotOne free 100/mo 起跑、$17/mo 升級點。詳見 `project_l4_datacenter_proxy_backlog.md`。註冊 + 1 env var 即接 |
...
| ~~Landing v1.6 — Duolingo 第二排~~ | ✅ SHIPPED 2026-04-28 PM（換 Burton 因 Duolingo brand color VLM 未上 — 還是延續視覺反差目標）| — |
| Landing D1 → D2 custom domain | BLOCKED on user | 等 user 買 domain；code 端 5 分鐘（vite base + CNAME + GH Pages 設定） |
| Scene Library Phase 3 — ProcessSteps | OPEN | 1→2→3 timeline scene、tech intent 主用；~3 hr eng；Phase 1+2 pattern 已驗 |
| Scene Library Phase 4 — DataChart | OPEN | 動畫長條/折線圖；hardsell 主用；~5 hr eng（最大 scope，需要 chart primitive）|
| `scripts/render-showcase-videos.mjs` fail-fast 改 graceful | LOW | T5 smoke 撞 1 job fail 整個 abort；換 try-catch 讓部分結果可用；非急 |
| Re-render showcase 加 VersusSplit | DEFER | VersusSplit 在 vercel/burton 觸發率不確定（spec §6 預期低）；等 prod 自然觸發再決定 |
...
- `project_landing_shipped.md` — 2026-04-28 landing 全鏈 ship 紀錄（含 v1.5 IntentVideoShowcase）+ D1→D2 swap 步驟
- `feedback_github_pages_first_run_setup.md` — Pages 第一次 deploy 必踩：Settings → Pages → Source = GitHub Actions
- `feedback_verify_api_fields_in_plan.md` — plan 內含跨模組 field 名（status enum / 欄位名 / env var）必須先 grep 驗證、別憑印象寫
- `project_scene_library_v17_backlog.md` — Phase 2-4 scene types committed during brainstorm（Phase 2 已 ship、Phase 3-4 待做）
- `project_scene_library_v17_phase1_shipped.md` — v1.7 Phase 1 完整 ship 紀錄 + T8 smoke 證 Claude 100% 採納 variants
- `project_scene_versus_split_phase2_shipped.md` — Phase 2 VersusSplit 完整 ship + smoke 證 Claude 嘗試使用、extractive 抓 fabrication = system as designed
...
8. **Plan 內含跨模組欄位名（API enum / DB column / env var）必須先 grep 驗證再寫**，否則 subagent 會盲執行燒 wall time + Anthropic credit。2026-04-28 撞過（`completed`/`outputUri` vs 真實 `done`/`videoUrl`）。
9. **長字串 extractive check 跨 sourceTexts entries 拼接** 已由 joinedPool fast-path（`e635832`）解掉。Claude 自然 normalize（`& → and`、smart quote、whitespace）也都對稱 fold，跨 source entry 拼接不再 false-reject。
...
1. **Bug #3 / L4 vendor 接 ScreenshotOne free tier** — 2026-04-28 vendor brainstorm 結論：先 ScreenshotOne 100/mo 免費起跑、$17/mo 升級點。1 個 env var 即接（pickTrack 架構已就緒）。詳見 `project_l4_datacenter_proxy_backlog.md`。
...
3. **Scene Library Phase 3-4** — ProcessSteps + DataChart 補齊「視覺多樣性」工程承諾。Phase 1+2 pattern 已完整驗、abbreviated review 流程也驗 work；Phase 3 大概 ~1.5 hr eng（同 Phase 2 規模）、Phase 4 ~5 hr（chart primitive 較大）。詳見 `project_scene_library_v17_backlog.md`。

4. **新 product feature** — User 在 Day 1 明確表態，backlog 清乾淨後想 focus user-facing feature。現在正是時機。README roadmap 候選：Scheduled re-renders / Custom brand kit / 9:16 vertical / Team workspaces / Webhook trigger。
